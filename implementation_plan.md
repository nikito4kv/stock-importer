...
- [x] Описать пользовательские сценарии:
  ...
  - [x] выход и повторный вход в том же managed Storyblocks-профиле;
...
- [x] Реализовать singleton manager браузерного профиля Storyblocks:
  - [x] создать managed профиль;
  - [x] создать предсказуемую структуру каталогов профиля (`user_data`, `downloads`, `diagnostics`);
  - [x] детерминированно выбрать legacy профиль при старом workspace;
  - [x] убрать multi-profile UX из основного runtime path.
...
- [ ] Выход и повторный вход в том же managed Storyblocks-профиле работает.
...
- [x] Сделать отдельную панель управления Storyblocks-сессией:
  - [x] открыть окно входа;
  - [x] импортировать внешнюю Chrome/Edge-сессию в тот же managed профиль;
  - [x] проверить состояние сессии;
  - [x] вручную подтвердить готовность;
  - [x] выйти;
  - [x] сбросить сессию без переключения профилей.
...
- [x] Подготовить onboarding:
  ...
  - [x] создание managed Storyblocks-профиля;
...
- [ ] Провести pilot rollout на нескольких сценариях и рабочих средах.
